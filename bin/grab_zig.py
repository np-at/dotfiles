...
def create_arg_parser():
    parser = argparse.ArgumentParser(
        description="Grab latest version of zig binary and download to designated directory"
    )
    parser.add_argument(
        "-d",
        "--directory",
        help="Directory to download zig binary to",
        required=False,
        default=None,
    )
    parser.add_argument(
        "-v",
        "--version",
        help="Version of zig to download; defaults to latest",
        required=False,
        default="master",
    )
    parser.add_argument(
        "-p",
        "--platform",
        help="Platform to download zig binary for; defaults to current",
        required=False,
    )
    parser.add_argument(
        "-o",
        "--os",
        help="OS to download zig binary for; defaults to current",
        required=False,
    )
    parser.add_argument(
        "-a",
        "--arch",
        help="Architecture to download zig binary for; defaults to current",
        required=False,
    )
    return parser
...
def main():
    ...
    if current_zig_version != zig_info["version"]:
        ...
        zig_download_file = os.path.join(
            temp_dir.name, f"zig-{args.version}-{platform}-{os_name}-{arch}.tar.xz"
        )
...
def untar(tar_file, dest_dir):
    ...
    Path(os.path.join(dest_dir, "zig")).symlink_to(
        os.path.join(dest_dir, "ziglib", "zig")
    )
